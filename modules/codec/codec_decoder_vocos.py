...
from .residual_fsq import ResidualFSQ
...
class CodecDecoderVocos(nn.Module):
    ...
    def forward(self, x, vq=True, skip_vq: bool = False, skip_project_in: bool = False):
        # If vq is True (used for encoder)
        if vq is True:
            # If skip_project_in, then we will skip the projection and pass the input directly to the quantizer.
            if skip_project_in:
                # x, q, commit_loss = self.quantizer(x)
                # x = x.permute(0, 2, 1)
                x, q = self.quantizer(x, skip_vq=skip_vq, skip_project_in=skip_project_in)
                x = x.permute(0, 2, 1)
                q = q.permute(0, 2, 1)
                return x, q, None
            # skip_vq: if True, then we will skip the vector quantization and return the output before quantization.
            elif skip_vq:
                # x, q, commit_loss = self.quantizer(x)
                x = x.permute(0, 2, 1)
                x, _ = self.quantizer(x, skip_vq=skip_vq, skip_project_in=skip_project_in)
                # x = x.permute(0, 2, 1)
                return x, None, None
            # normal vq behavior
            else:
        ...
        # If vq is False (used for decoder), then we will skip the quantization and pass the input directly to the backbone.
        else:
            x = self.backbone(x)
            x, _ = self.head(x)
