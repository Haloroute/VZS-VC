...
class ResidualFSQ(Module):
    ...
    def forward(
        ...
        skip_vq: bool = False,
        skip_project_in: bool = False
    ):
        num_quant, quant_dropout_multiple_of, device = self.num_quantizers, self.quantize_dropout_multiple_of, x.device
        if skip_vq and skip_project_in:
            warnings.warn('Both skip_vq and skip_project_in are set to True.\
                           When this happens, the function will skip the projection in and also skip vector quantization, which means the output will be the same as the input.\
                           This is likely not what you intended. Please set either skip_vq or skip_project_in to False.', UserWarning)
        
        # If not skip_project_in, then we will do the projection in this forward pass. This is the default behavior.
        if not skip_project_in:
            x = self.project_in(x)
            # If skip_vq, then we will return the output before vector quantization
        if skip_vq:
            return x, None
