...
# simple versions of SwooshL that also return the derivative, used in
# ActivationDropoutAndLinearFunction.
def SwooshLForwardAndDeriv(x: Tensor):
    # swoosh_l(x) = log(1 + exp(x-4)) - 0.08*x - 0.035
    # derivative = sigmoid(x-4) - 0.08
    return SwooshLForward(x), torch.sigmoid(x - 4.0) - 0.08


# simple versions of SwooshR that also return the derivative, used in
# ActivationDropoutAndLinearFunction.
def SwooshRForwardAndDeriv(x: Tensor):
    # swoosh_r(x) = log(1 + exp(x-1)) - 0.08*x - 0.313261687
    # derivative = sigmoid(x-1) - 0.08
    return SwooshRForward(x), torch.sigmoid(x - 1.0) - 0.08
