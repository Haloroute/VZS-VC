# Main training loop
import datasets, math, os, random, torch
import numpy as np

from dataclasses import asdict
from datasets import DatasetBuilder, IterableDataset, IterableDatasetDict
...
def train_model():
    ...
    dataset: IterableDatasetDict = datasets.load_dataset(dataset_config.path, streaming=True)
    train_dataset, val_dataset = dataset[dataset_config.train_split], dataset[dataset_config.val_split]
    ...
    train_loader = StatefulDataLoader(
        ...
        collate_fn=lambda batch: collate_fn(batch, dataset_config),
        pin_memory=True
    )
    ...
    model: MeanFlowsGenerator = load_generator(train_config.device).train()
    loss_fn: MeanFlowsAdaptedLoss = load_loss_fn(train_config.device).train()
    model = model.compile()
    loss_fn = loss_fn.compile()
    ...
    for epoch in range(1, train_config.n_epochs + 1):
        ...
        try:
            for i, batch in (t := tqdm(enumerate(train_loader), desc="Training", total=math.ceil(n_train_samples / train_config.batch_size), leave=False)):
                ...
                with autocast(device_type=train_config.device, dtype=torch.bfloat16, enabled=train_config.use_amp):
            ...
            timestamp = torch.tensor(int(torch.time.time())).item()
        ...
        if epoch % validation_config.validate_every_n_epochs == 0: # Validate every few epochs
            ...
            with torch.inference_mode():
                for i, batch in tqdm(enumerate(val_loader), desc="Validation", total=math.ceil(n_val_samples / validation_config.batch_size), leave=False):
                    ...
                    with autocast(device_type=validation_config.device, dtype=torch.bfloat16, enabled=validation_config.use_amp):
                    ...
                    val_loss += loss.item() * batch['timbre'].size(0)
